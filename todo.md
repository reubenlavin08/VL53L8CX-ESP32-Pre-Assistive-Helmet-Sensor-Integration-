...
## Active request queue (2026-06-07 — work ONE at a time, confirm before next)

Per Reuben's workflow rule: log every request here BEFORE starting, do ONE, report+confirm, then raise the next. He dumps ideas as they come; we work them sequentially, no multitasking.

- [x] **A. Visualizer 1 FPS regression.** RESOLVED — was stale TCP clients + ESP brownout, AND the `FREERTOS_HZ=100` bus-starvation bug (see 2026-06-08 below). After the FREERTOS fix the viz holds a steady 16 FPS on a clean boot.
- [x] **B. Green LED meaning.** Firmware drives NO status LED — the green LED is a BOARD indicator (serial-TX / USB-JTAG activity), not firmware. Confirmed.
- [ ] **C. CV-robotics course catch-up.** Untouched since the first ToF install. Review where he left off (`C:\claude-projects\cv-robotics-course\BRIEFING.md`) + what to go over. (TaskList has Module 0/5/3 pending.)
- [ ] **D. ToF↔ToF interference — DECISION.** Research done: largely a NON-ISSUE for this geometry (~35° apart; histogram rejects cross-light as ambient noise, not phantom targets); ST's SYNC pin is the first-party fix if ever needed. Optional 10-min A/B test (log `target_status` + sigma with the other sensor ON vs OFF at <60 cm). Decide whether to bother.
- [ ] **E. 3D-viz approach — DECISION.** Research done: recommend switching the viz to **Rerun** (rerun.io) — purpose-built for streamed sensor + transform-tree + recording/playback; deletes the hand-rolled render loop/threads/transforms. ~1–2 sessions; keep PyQtGraph until Rerun reaches parity. Decide: pivot vs. keep refining PyQtGraph.
- [ ] **F. Visualizer refactor to best practices** — folds into E (Rerun) or hardening PyQtGraph (latest-frame handoff already present; reconnect + exception isolation improved 2026-06-07).
- [x] **G. IMU yaw drift.** DONE (2026-06-08). Empirically A/B-tested the magnetometer: the haptic motors corrupt it (mag accuracy High→Low, ~60° yaw jumps) at ~5 cm spacing → went **mag-free `SH2_ARVR_STABILIZED_GRV`**. Added gyro-ZRO + accel calibration (`sh2_setCalConfig`) and DCD persistence (`sh2_setDcdAutoSave`). Dynamic drift +2.24°→+0.82° (~63%). Recenter button = north substitute. Full RCA in `docs/DEVLOG.md`. **Open follow-ups:** confirm drift gain with 2+ more `after_levers` runs (N=1 so far); verify DCD-persistence across a power cycle.

**Researched 2026-06-08 (evening) — Phase 2/3 sensor suite (full writeup: `docs/camera-and-multisensor-layout.md`):** Camera HBV-1716WA/OV2710 datasheets downloaded (`docs/datasheets/camera/`). Key findings: WA lens = 140° **diagonal** → extrapolated **HFOV ~109° / VFOV ~67°** from HBVCAM's lens ladder (CALIBRATE to confirm); **1080p is MJPEG-only** (YUY2 maxes at 640×480); OV2710 is **rolling shutter**. ToF↔ToF interference **properly researched** (was overstated before): real for dToF generally, but L8CX = noise-floor + `target_status`-flagged, NOT phantom obstacles; fix = edge-to-edge or temporal stagger. New **two-ToF layout** designed: each sensor **22.5° off-center (45° apart) = 90° H**, tilted **22.5° down** (FoV 0° to −45°), + HC-SR04 ultrasonics for overhead/near-low gaps. **This left/right design supersedes the top-up/bottom-down build (item below) — still a proposal, not built.** Open: ultrasonic placement (1up+1down rec.), camera calibration, max-overlap-vs-max-coverage aiming decision. **Calibration + depth↔pixel fusion also researched** (`docs/camera-calibration-and-depth-fusion.md`): 140° → use `cv2.fisheye` (Kannala-Brandt) + ChArUco, NOT pinhole; for fusion keep raw image + `fisheye.projectPoints` the 64 ToF zones via measured extrinsics; skip depth-completion nets (overkill at 64 pts) → projection + detection-gating. Leaders surveyed (KITTI completion, BEVFusion/PointPainting, Apple/RealSense). **Round 2 (3 research agents, `docs/camera-compute-and-fusion-options.md`):** (1) **Compute host** — ESP32-S3 can't host the cam (Full-Speed USB → VGA); rec = laptop dev → **Pi 5 + Hailo-8L (~$130/10 W, YOLOv8s 25–35 FPS)** as vision brain, S3 stays reflex/haptic. (2) **Fusion progression** A overlay+reflex → B detection-gating → C mono-depth+ToF-anchor (Depth Anything → dense metric depth over full 140°); skip generic completion nets. (3) **UM3109-VERIFIED CORRECTIONS:** VL53L8CX **HAS a dedicated SYNC pin (B1)** for multi-sensor interference (`vl53l8cx_set_external_sync_pin_enable`, §4.15) — the real dual-ToF fix; **no dedicated interference flag** (degrades to low-conf `target_status`, only 5=100% valid); OV2710 rolling-shutter confirmed; 140°=diagonal confirmed.

**Shipped this session (2026-06-08):** IMU yaw-drift resolved end-to-end — magnetometer A/B test → **mag-free AR/VR-Stabilized GRV** decision (motors corrupt the mag: High→Low accuracy, ~60° jumps); found + fixed the **`FREERTOS_HZ=100` bug** (IMU task's 2 ms delay rounded to 0 → spun → starved the ToF loop on some boots = the old "~20% warm-reboot flake") by setting it to 1000; added **gyro-ZRO + accel calibration + DCD persistence**; dynamic drift +2.24°→+0.82°. Decoupled `Q:` from the ToF frame gate. New test tools: `imu_capture.py`, `imu_drift_dynamic.py`. Full RCA in `docs/DEVLOG.md`.

**Shipped 2026-06-07:** BNO085 IMU fully working at ~100 Hz on the shared I²C bus alongside both ToF sensors — three stacked bugs fixed (wrong addr → **0x4A**; missed one-time SHTP advertisement → **soft-reset** in `hal_open`; hub starvation during ToF firmware upload → **bring IMU up after ToF init + dedicated 500 Hz task + mutex**). `Q:` quaternion integrated into the visualizer (head-attached geometry rotates with orientation; "Recenter IMU" button). Full RCA in `docs/DEVLOG.md`. Fixed a viz `NameError` (`side`→`self.side`) + exception isolation on the IMU path.
...
- Sensor I²C init flake (~20% of warm reboots) — **LIKELY ROOT-CAUSED 2026-06-08:** `FREERTOS_HZ=100` made the IMU task's 2 ms delay round to 0 → it spun and starved the ToF loop at the shared mutex. Fixed by `FREERTOS_HZ=1000`. Watch whether the flake recurs after this.
...
- ESP I²C sensor init fails ~20% of the time on warm reboot. Mitigation: host-side 3× retry in `run_one_test.ps1`. **2026-06-08: likely root-caused as the `FREERTOS_HZ=100` IMU-task spin starving the ToF loop — fixed by `FREERTOS_HZ=1000`; verify the flake is gone.** Also: USB-Serial-JTAG re-enumerates on an RTS reset, so don't RTS-reset to start a capture — attach to the running stream or reset via `esptool`.
